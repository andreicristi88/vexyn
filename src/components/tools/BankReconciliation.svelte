      ...
      <div class="space-y-3">
        {#if !slot.grid}
          <div class="dropzone-tint border-2 border-dashed rounded-xl p-8 text-center {slot.dragOver ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)]'}"
            ...
            {#if si === 0}
              <input bind:this={inputL} type="file" accept=".csv,.tsv,.txt,text/csv" class="hidden" on:change={(e) => { const t = e.currentTarget; if (t.files?.[0]) loadFile(slot, t.files[0]); t.value = ''; }} />
            {:else}
