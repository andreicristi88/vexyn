<script lang="ts">
  ...
  const items = $derived(result ? detectRecurring(result.txns, direction) : []);
  const monthlyTotal = $derived(round2(items.reduce((s, r) => s + r.monthlyCost, 0)));
...
<div class="space-y-4">
  {#if !grid}
    <div class="dropzone-tint border-2 border-dashed rounded-xl p-10 text-center {dragOver ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)]'}"
      ...
      <p class="text-xs text-[color:var(--color-text-dim)] mt-3">Tip: export 3+ months so a monthly pattern is visible.</p>
    </div>
  {/if}
  ...
  {#if grid}
    ...
    {#if !ready}
      <p class="text-sm text-[color:var(--color-text-mute)]">Choose a <strong>Date</strong>, an amount, and the <strong>Description</strong> column to find repeating payments.</p>
    {:else if result}
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="inline-flex rounded-lg border border-[color:var(--color-border)] overflow-hidden text-sm">
          <button class="px-4 py-2 {direction === 'out' ? 'bg-[color:var(--color-brand-500)] text-white' : 'text-[color:var(--color-text-mute)]'}" on:click={() => (direction = 'out')}>Payments out</button>
          <button class="px-4 py-2 {direction === 'in' ? 'bg-[color:var(--color-brand-500)] text-white' : 'text-[color:var(--color-text-mute)]'}" on:click={() => (direction = 'in')}>Payments in</button>
        </div>
        {#if items.length}
          <p class="text-sm text-[color:var(--color-text-mute)]"><span class="font-semibold text-[color:var(--color-text)]">{items.length}</span> recurring · about <span class="font-semibold {direction === 'out' ? 'text-[color:var(--color-danger)]' : 'text-[color:var(--color-success)]'}">{money(monthlyTotal)}</span>/month</p>
        {/if}
      </div>

      {#if items.length === 0}
        <div class="p-6 rounded-xl bg-[color:var(--color-surface)] border border-[color:var(--color-border)] text-center">
          <p class="text-[color:var(--color-text-mute)]">No recurring payments {direction === 'out' ? 'out' : 'in'} found. A repeating payment needs at least two charges to the same payee at a regular interval — try a longer date range.</p>
        </div>
      {:else}
          ...
          {#each items as s}
            <div class="flex items-center gap-3 p-4 bg-[color:var(--color-surface)]">
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-[color:var(--color-text-mute)] mt-0.5">{s.count} payments · {s.first} → {s.last}</p>
              </div>
              <div class="text-right whitespace-nowrap">
                <p class="font-mono text-sm {direction === 'out' ? 'text-[color:var(--color-danger)]' : 'text-[color:var(--color-success)]'}">{money(s.amount)}</p>
                <p class="text-[11px] text-[color:var(--color-text-dim)]">≈ {money(s.monthlyCost)}/mo</p>
              </div>
            </div>
          {/each}
        </div>
        <p class="text-xs text-[color:var(--color-text-dim)]">A payment is flagged when the same payee recurs at a stable amount and a regular interval; “unconfirmed” means only two matched. Best-effort — verify against your statement.</p>
      {/if}
    {/if}
  {/if}
</div>
