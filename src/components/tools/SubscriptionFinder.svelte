<script lang="ts">
  ...
  const monthlyTotal = $derived(round2(subs.reduce((s, r) => s + r.monthlyCost, 0)));
  const annualTotal = $derived(round2(monthlyTotal * 12));
  ...
  function autoConfigure(g: Grid) {
    ...
    if (map.date >= 0) {
      ...
      if (s) {
        ...
        else { const c = s.trim().split(/[\/.\-]/).map((x) => parseInt(x, 10)); dateFormat = c[0] > 12 ? 'eu' : c[1] > 12 ? 'us' : 'us'; }
      }
    }
  ...
  const CAD_LABEL: Record<string, string> = { weekly: 'Weekly', biweekly: 'Every 2 weeks', monthly: 'Monthly', quarterly: 'Quarterly', yearly: 'Yearly' };
</script>
...
<div class="space-y-4">
  {#if !grid}
    <div class="dropzone-tint border-2 border-dashed rounded-xl p-10 text-center {dragOver ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)]'}"
      ...
      <p class="text-[color:var(--color-text-mute)] mb-3">Drop your bank statement CSV or Excel file here, or</p>
      ...
      <p class="text-xs text-[color:var(--color-text-dim)] mt-3">Tip: export 3+ months so a monthly charge shows up as a pattern.</p>
    </div>
  {/if}
  ...
  {#if grid}
    ...
    {#if !ready}
      <p class="text-sm text-[color:var(--color-text-mute)]">Choose a <strong>Date</strong>, an amount, and the <strong>Description</strong> column to scan for subscriptions.</p>
    {:else if result}
      {#if result.skipped.length}
        <div class="p-3 rounded-lg bg-[color:var(--color-accent-500)]/10 border border-[color:var(--color-accent-500)]/30 text-xs text-[color:var(--color-text-mute)]">
          {result.skipped.length} row{result.skipped.length !== 1 ? 's' : ''} skipped (unreadable date or amount). Check the date format and decimal separator above.
        </div>
      {/if}

      {#if subs.length === 0}
        <div class="p-6 rounded-xl bg-[color:var(--color-surface)] border border-[color:var(--color-border)] text-center">
          <p class="text-[color:var(--color-text-mute)]">No recurring charges found. Recurring charges need at least two payments to the same merchant at a regular interval — try exporting a longer date range.</p>
        </div>
      {:else}
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
          <div class="p-4 rounded-xl bg-[color:var(--color-surface)] border border-[color:var(--color-border)]">
            <p class="text-xs text-[color:var(--color-text-mute)] mb-1">Recurring charges</p>
            <p class="text-xl font-bold">{subs.length}</p>
          </div>
          <div class="p-4 rounded-xl bg-[color:var(--color-surface)] border border-[color:var(--color-border)]">
            <p class="text-xs text-[color:var(--color-text-mute)] mb-1">Est. per month</p>
            <p class="text-xl font-bold text-[color:var(--color-danger)]">{money(monthlyTotal)}</p>
          </div>
          <div class="p-4 rounded-xl bg-[color:var(--color-surface)] border border-[color:var(--color-border)]">
            <p class="text-xs text-[color:var(--color-text-mute)] mb-1">Est. per year</p>
            <p class="text-xl font-bold text-[color:var(--color-danger)]">{money(annualTotal)}</p>
          </div>
        </div>

        <div class="rounded-xl border border-[color:var(--color-border)] divide-y divide-[color:var(--color-border)] overflow-hidden">
          {#each subs as s}
            <div class="flex items-center gap-3 p-4 bg-[color:var(--color-surface)]">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                  <span class="font-medium truncate">{s.merchant}</span>
                  <span class="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-[color:var(--color-brand-500)]/12 text-[color:var(--color-brand-400)] border border-[color:var(--color-brand-500)]/25">{CAD_LABEL[s.cadence]}</span>
                  {#if !s.confident}<span class="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-[color:var(--color-surface-2)] text-[color:var(--color-text-dim)] border border-[color:var(--color-border)]" title="Only two matching charges — could be a coincidence.">unconfirmed</span>{/if}
                </div>
                <p class="text-xs text-[color:var(--color-text-mute)] mt-0.5">{s.count} charges · {s.first} → {s.last}</p>
              </div>
              <div class="text-right whitespace-nowrap">
                <p class="font-mono text-sm text-[color:var(--color-danger)]">{money(s.amount)}</p>
                <p class="text-[11px] text-[color:var(--color-text-dim)]">≈ {money(s.monthlyCost)}/mo</p>
              </div>
            </div>
          {/each}
        </div>
        <p class="text-xs text-[color:var(--color-text-dim)]">Estimates. A charge is flagged when the same merchant is billed a stable amount at a regular interval; “unconfirmed” means only two charges matched. Always confirm against your statement before cancelling anything.</p>
      {/if}
    {/if}
  {/if}
</div>
