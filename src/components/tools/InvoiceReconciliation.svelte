<script lang="ts">
  ...
  type Slot = {
    ...
    map: StatementMap; dateFormat: DateFormat; decimal: '.' | ','; dragOver: boolean; error: string;
  };
  ...
  let Inv = $state<Slot>(newSlot('Invoices issued', 'what you billed'));
  let Pay = $state<Slot>(newSlot('Payments received', 'what came in'));
  let amountTolerance = $state(0);
  let dateToleranceDays = $state(30);
  let view = $state<'unpaid' | 'unapplied' | 'matched'>('unpaid');
  let sym = $state('$');

  let inputI: HTMLInputElement;
  let inputP: HTMLInputElement;

  const txnsI = $derived(slotReady(Inv) ? buildTransactions(Inv.grid!.rows, Inv.map, Inv.dateFormat, Inv.decimal) : null);
  const txnsP = $derived(slotReady(Pay) ? buildTransactions(Pay.grid!.rows, Pay.map, Pay.dateFormat, Pay.decimal) : null);
  const result = $derived(txnsI && txnsP ? matchTransactions(txnsI.txns, txnsP.txns, { amountTolerance, dateToleranceDays }) : null);

  const invoicedSum = $derived(txnsI ? sumAmount(txnsI.txns) : 0);
  const paidSum = $derived(txnsP ? sumAmount(txnsP.txns) : 0);
  const unpaidSum = $derived(result ? sumAmount(result.onlyA) : 0);
  const unappliedSum = $derived(result ? sumAmount(result.onlyB) : 0);

  const viewRows = $derived.by<Txn[]>(() => {
    if (!result) return [];
    if (view === 'matched') return result.matched.map((p) => p.a);
    if (view === 'unpaid') return result.onlyA;
    return result.onlyB;
  });
  ...
  function autoConfigure(s: Slot, g: Grid) {
    const debit = guess(g.headers, [/debit|paid out|withdrawal|money out|\bdr\b/]);
    const credit = guess(g.headers, [/credit|paid in|deposit|money in|\bcr\b/]);
    const amount = guess(g.headers, [/amount|total|net|value|due|paid|gross/]);
    const useSplit = debit >= 0 && credit >= 0 && amount < 0;
    s.map = { ...DEFAULT_MAP, amountMode: useSplit ? 'split' : 'single', date: guess(g.headers, [/date|created|issued|paid|due|time/]), amount, debit, credit, description: guess(g.headers, [/invoice|number|customer|client|payee|name|desc|reference|memo/]) };
  ...
  function download(fmtId: string) {
    ...
    a.href = url; a.download = `${view}.${fmt.ext}`; a.click();
...
<div class="space-y-4">
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    {#each [Inv, Pay] as slot, si (slot.label)}
      <div class="space-y-3">
        {#if !slot.grid}
  ...
  {#if Inv.grid && Pay.grid}
    <div class="flex flex-wrap items-center gap-4 p-4 rounded-xl bg-[color:var(--color-surface)] border border-[color:var(--color-border)]">
      ...
      <label class="text-sm flex items-center gap-2">paid within
        <select bind:value={dateToleranceDays} class="px-2 py-1 rounded-lg bg-[color:var(--color-surface-2)] border border-[color:var(--color-border)] text-sm"><option value={7}>7 days</option><option value={14}>14 days</option><option value={30}>30 days</option><option value={60}>60 days</option><option value={90}>90 days</option></select>
      </label>
  ...
  {#if !result}
    <p class="text-sm text-[color:var(--color-text-mute)]">Load your invoices and the payments received, map a <strong>Date</strong> and an amount column in each, to see what is paid and what is outstanding.</p>
  {:else}
    <div class="p-5 rounded-xl border {result.onlyA.length === 0 ? 'border-[color:var(--color-success)]/40 bg-[color:var(--color-success)]/8' : 'border-[color:var(--color-border)] bg-[color:var(--color-surface)]'}">
      {#if result.onlyA.length === 0}
        <p class="text-lg font-bold text-[color:var(--color-success)]">All invoices paid ✓</p>
        <p class="text-sm text-[color:var(--color-text-mute)] mt-1">Every invoice has a matching payment.</p>
      {:else}
        <p class="text-lg font-bold"><span class="text-[color:var(--color-danger)]">{money(unpaidSum)}</span> outstanding across {result.onlyA.length} invoice{result.onlyA.length !== 1 ? 's' : ''}</p>
        <p class="text-sm text-[color:var(--color-text-mute)] mt-1">Invoiced {money(invoicedSum)} · received {money(paidSum)}. {result.onlyB.length} payment{result.onlyB.length !== 1 ? 's' : ''} could not be matched to an invoice.</p>
      {/if}
    </div>

    <div class="grid grid-cols-3 gap-3">
      <button class="p-4 rounded-xl border text-left transition-colors {view === 'unpaid' ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)] bg-[color:var(--color-surface)]'}" on:click={() => (view = 'unpaid')}>
        <p class="text-xs text-[color:var(--color-text-mute)] mb-1">Unpaid invoices</p>
        ...
        <p class="text-[11px] text-[color:var(--color-text-dim)] mt-0.5">{money(unpaidSum)}</p>
      </button>
      <button class="p-4 rounded-xl border text-left transition-colors {view === 'unapplied' ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)] bg-[color:var(--color-surface)]'}" on:click={() => (view = 'unapplied')}>
        <p class="text-xs text-[color:var(--color-text-mute)] mb-1">Unmatched payments</p>
        ...
        <p class="text-[11px] text-[color:var(--color-text-dim)] mt-0.5">{money(unappliedSum)}</p>
      </button>
      <button class="p-4 rounded-xl border text-left transition-colors {view === 'matched' ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)] bg-[color:var(--color-surface)]'}" on:click={() => (view = 'matched')}>
        <p class="text-xs text-[color:var(--color-text-mute)] mb-1">Paid</p>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-xs border-collapse">
          <thead><tr class="bg-[color:var(--color-surface-2)]">
            ...
            <th class="text-left px-3 py-2 font-semibold border-b border-[color:var(--color-border)]">Reference</th>
          </tr></thead>
          <tbody>
            {#each viewRows.slice(0, 50) as t}
              <tr class="border-b border-[color:var(--color-border)] last:border-0">
                ...
                <td class="px-3 py-1.5 text-right whitespace-nowrap font-mono">{money(t.amount)}</td>
            ...
            {/each}
            {#if viewRows.length === 0}<tr><td colspan="3" class="px-3 py-4 text-center text-[color:var(--color-text-mute)]">Nothing here.</td></tr>{/if}
          </tbody>
        </table>
      </div>
