<script lang="ts">
  ...
  let L = $state<Slot>(newSlot('Your ledger', 'what your books say'));
  let Bk = $state<Slot>(newSlot('Bank statement', 'what the bank says'));
  let amountTolerance = $state(0);
  let dateToleranceDays = $state(3);
  let view = $state<'outstanding' | 'missing' | 'matched'>('outstanding');
  let sym = $state('$');

  let inputL: HTMLInputElement;
  let inputB: HTMLInputElement;

  const txnsL = $derived(slotReady(L) ? buildTransactions(L.grid!.rows, L.map, L.dateFormat, L.decimal) : null);
  const txnsB = $derived(slotReady(Bk) ? buildTransactions(Bk.grid!.rows, Bk.map, Bk.dateFormat, Bk.decimal) : null);
  const result = $derived(txnsL && txnsB ? matchTransactions(txnsL.txns, txnsB.txns, { amountTolerance, dateToleranceDays }) : null);

  const ledgerSum = $derived(txnsL ? sumAmount(txnsL.txns) : 0);
  const bankSum = $derived(txnsB ? sumAmount(txnsB.txns) : 0);
  const difference = $derived(round2(bankSum - ledgerSum));
  const outstandingSum = $derived(result ? sumAmount(result.onlyA) : 0); // in ledger, not bank
  const missingSum = $derived(result ? sumAmount(result.onlyB) : 0); // in bank, not ledger
  const reconciled = $derived(!!result && result.onlyA.length === 0 && result.onlyB.length === 0);

  const viewRows = $derived.by<Txn[]>(() => {
    if (!result) return [];
    if (view === 'matched') return result.matched.map((p) => p.a);
    if (view === 'outstanding') return result.onlyA;
    return result.onlyB;
  });
...
<div class="space-y-4">
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    {#each [L, Bk] as slot, si (slot.label)}
      <div class="space-y-3">
        {#if !slot.grid}
          <div class="dropzone-tint border-2 border-dashed rounded-xl p-8 text-center {slot.dragOver ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)]'}"
            ...
            <button class="px-4 py-2 rounded-lg bg-[color:var(--color-brand-500)] hover:bg-[color:var(--color-brand-600)] text-white text-sm font-medium transition-colors" on:click={() => (si === 0 ? inputL : inputB).click()}>Choose file</button>
            {#if si === 0}
              <input bind:this={inputL} type="file" accept={TABULAR_ACCEPT} class="hidden" on:change={(e) => { const t = e.currentTarget; if (t.files?.[0]) loadFile(slot, t.files[0]); t.value = ''; }} />
            {:else}
              <input bind:this={inputB} type="file" accept={TABULAR_ACCEPT} class="hidden" on:change={(e) => { const t = e.currentTarget; if (t.files?.[0]) loadFile(slot, t.files[0]); t.value = ''; }} />
            {/if}
          </div>
        {:else}
  ...
  {#if !result}
    <p class="text-sm text-[color:var(--color-text-mute)]">Load your ledger and the bank statement, map a <strong>Date</strong> and an amount column in each, and the reconciliation appears.</p>
  {:else}
    <!-- Reconciliation verdict -->
    <div class="p-5 rounded-xl border {reconciled ? 'border-[color:var(--color-success)]/40 bg-[color:var(--color-success)]/8' : 'border-[color:var(--color-border)] bg-[color:var(--color-surface)]'}">
      {#if reconciled}
        <p class="text-lg font-bold text-[color:var(--color-success)]">Reconciled ✓</p>
        <p class="text-sm text-[color:var(--color-text-mute)] mt-1">Every transaction in your ledger has a match on the bank statement, and vice versa.</p>
      {:else}
        <p class="text-lg font-bold">Off by <span class="{difference < 0 ? 'text-[color:var(--color-danger)]' : 'text-[color:var(--color-text)]'}">{signed(difference)}</span></p>
        <p class="text-sm text-[color:var(--color-text-mute)] mt-1">Bank {money(bankSum)} − ledger {money(ledgerSum)}. Explained by {result.onlyA.length + result.onlyB.length} unmatched item{result.onlyA.length + result.onlyB.length !== 1 ? 's' : ''} below — clear them and it reconciles to zero.</p>
      {/if}
    </div>

    <!-- Buckets -->
    <div class="grid grid-cols-3 gap-3">
      <button class="p-4 rounded-xl border text-left transition-colors {view === 'outstanding' ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)] bg-[color:var(--color-surface)]'}" on:click={() => (view = 'outstanding')}>
        <p class="text-xs text-[color:var(--color-text-mute)] mb-1">In ledger, not bank</p>
        ...
        <p class="text-[11px] text-[color:var(--color-text-dim)] mt-0.5">{signed(outstandingSum)} · uncleared</p>
      </button>
      <button class="p-4 rounded-xl border text-left transition-colors {view === 'missing' ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)] bg-[color:var(--color-surface)]'}" on:click={() => (view = 'missing')}>
        <p class="text-xs text-[color:var(--color-text-mute)] mb-1">On bank, not ledger</p>
        ...
        <p class="text-[11px] text-[color:var(--color-text-dim)] mt-0.5">{signed(missingSum)} · to record</p>
      </button>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-xs border-collapse">
          ...
          <tbody>
            ...
            {/each}
            {#if viewRows.length === 0}<tr><td colspan="3" class="px-3 py-4 text-center text-[color:var(--color-text-mute)]">Nothing here — this side is fully reconciled.</td></tr>{/if}
          </tbody>
        </table>
      </div>
