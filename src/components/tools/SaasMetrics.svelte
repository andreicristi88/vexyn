<script lang="ts">
  ...
  import { isStripeSubscriptions, saasMetrics } from '../../lib/stripe';
  ...
  let decimal = $state<'.' | ','>('.');
  let notSubs = $state(false);

  let fileInput: HTMLInputElement;

  const m = $derived(grid ? saasMetrics(grid, decimal) : null);
  ...
  function loadText(text: string, name: string) {
    ...
    notSubs = !isStripeSubscriptions(res.grid.headers);
  ...
  function reset() { grid = null; fileName = ''; error = ''; notSubs = false; }
...
<div class="space-y-4">
  {#if !grid}
    <div class="dropzone-tint border-2 border-dashed rounded-xl p-10 text-center {dragOver ? 'border-[color:var(--color-brand-500)] bg-[color:var(--color-brand-500)]/5' : 'border-[color:var(--color-border)]'}"
      ...
      <p class="text-[color:var(--color-text-mute)] mb-3">Drop your Stripe subscriptions export here, or</p>
      ...
      <p class="text-xs text-[color:var(--color-text-dim)] mt-3">Billing → Subscriptions → Export.</p>
    </div>
  {/if}
  ...
  {#if grid && m}
    ...
    {#if notSubs}<div class="p-3 rounded-lg bg-[color:var(--color-accent-500)]/10 border border-[color:var(--color-accent-500)]/30 text-xs text-[color:var(--color-text-mute)]">This does not look like a Stripe subscriptions export (needs Status, Interval and Amount columns). Numbers may be off — check the export.</div>{/if}
    ...
    {/if}
    <p class="text-xs text-[color:var(--color-text-dim)]">MRR normalizes every paying subscription to a monthly figure (yearly plans ÷ 12, and so on) times quantity; trials are counted but excluded. “Lost MRR” is the monthly value of canceled subscriptions in this file — a snapshot, not a cohort churn rate.</p>
  {/if}
</div>
